 ...
 * may be used to even report errors across networks.
 ...
 * on {@link com.pair.util.NavigationManager} to know the state of the application.
 *
...
public class ErrorCenter {
     ...
     *                     a particular error or cancel it at all..
     * @param errorMessage the message to be shown to the user
     * @param timeout      when the message should the message be discarded can't be less than 10000 (10 seconds)
     ...
     */
    public static void reportError(String errorId, String errorMessage, long timeout) {
        if (timeout != INDEFINITE) {
            timeout = Math.max(timeout, DEFAULT_TIMEOUT);
        }
        if (NavigationManager.getCurrentActivityState().equals(NavigationManager.States.RESUMED)) {
            try {
                showError(errorMessage);
            } catch (NoVisibleActivityException e) {
    ...
    private static void showError(final String errorMessage) throws NoVisibleActivityException {
        final FragmentActivity activity = NavigationManager.getCurrentActivity();

        activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                try {
                    UiHelpers.showErrorDialog(activity, errorMessage);
                } catch (Exception e) { //normally happen when attempting show a dialog fragment after onSavedInstantState
                    Log.e(TAG, "error while showing dialog fragment, falling back to Alert dialog");
                    UiHelpers.showPlainOlDialog(activity, errorMessage);
                }
                waitingError = null;
            }
        });
    ...
    private static class Error {
        ...
        Error(String id, String message, long timeout) {
            this.id = id;
