...
//        } while (cursor < size);
    ...
    protected void dispatchToUser(Message message) {
        ...
        WritableMessage writableMessage = new WritableMessage(message.getTo(), object.toString());
        writableMessage.addHeader(Message.SENDABLE_MESSAGE, "1");
        messageClient.send(writableMessage);
    }
    ...
    private void onSent(com.sinch.android.rtc.messaging.Message message) {
        if (message.getHeaders().containsKey(Message.SENDABLE_MESSAGE)) {
            ...
            //persist the id of this sinch message and the messageId it maps. see the how we make delivery report
    ...
    private void onDelivered(MessageDeliveryInfo messageDeliveryInfo) {
        SharedPreferences preferences = Config.getPreferences(MESSAGE_ID_MAPPER);
        String messageId = messageDeliveryInfo.getMessageId();
        String ourMessageId = preferences.getString(messageId, null);
        preferences.edit().remove(messageDeliveryInfo.getMessageId()).apply();
